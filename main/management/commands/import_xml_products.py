...
logging.basicConfig(
    level=logging.DEBUG,  # Измените с INFO на DEBUG для более подробных логов
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.critical(f"Не удалось загрузить связи категорий: {str(e)}", exc_info=True)
            self.stdout.write(self.style.ERROR(f"Ошибка загрузки tree.xml: {str(e)}"))
    ...
    def load_category_mappings(self):
        """Загружает связи товаров и категорий из tree.xml"""
        tree_url = "https://api2.gifts.ru/export/v2/catalogue/tree.xml"

        try:
            headers = {'User-Agent': 'Mozilla/5.0', 'Accept-Encoding': 'gzip'}

            logger.debug(f"Начинаю загрузку связей категорий из {tree_url}")

            for attempt in range(3):
                try:
                    response = self.session.get(tree_url, headers=headers, timeout=30)
                    response.raise_for_status()
                    break
                except (requests.exceptions.RequestException, requests.exceptions.Timeout) as e:
                    if attempt == 2:
            ...
            # Parse all product-category relationships
            for product_elem in root.findall('.//product'):
                category_id = product_elem.find('page')
                product_id = product_elem.find('product')

                if category_id is not None and product_id is not None:
                    category_id = category_id.text.strip()
                    product_id = product_id.text.strip()
                    self.product_to_category[product_id] = category_id
                    logger.debug(f"Найдена связь: товар {product_id} -> категория {category_id}")

            logger.info(f"Загружено {len(self.product_to_category)} связей товар→категория из tree.xml")
            self.stdout.write(
                self.style.SUCCESS(f"Загружено {len(self.product_to_category)} связей товаров с категориями"))

        except Exception as e:
            logger.error(f"Ошибка загрузки tree.xml: {str(e)}", exc_info=True)
            raise
    ...
    def process_category(self, product, xml_product):
        """Привязывает товар к категории на основе словаря product_to_category"""
        product_id = product.find('product_id').text.strip()

        if product_id in self.product_to_category:
            category_id = self.product_to_category[product_id]
            try:
                category = Category.objects.get(xml_id=category_id)
                xml_product.categories.add(category)
                logger.debug(
                    f"Товар {product_id} успешно привязан к категории {category_id} ({category.name if category else 'нет названия'})")
                self.stdout.write(
                    self.style.SUCCESS(f"Товар {product_id} → Категория: {category.name if category else category_id}"))
            except Category.DoesNotExist:
                logger.warning(f"Категория {category_id} не найдена в базе для товара {product_id}")
                self.stdout.write(self.style.WARNING(f"Категория {category_id} не найдена для товара {product_id}"))
        else:
            logger.warning(f"Для товара {product_id} не найдена категория в tree.xml")
            self.stdout.write(self.style.WARNING(f"Для товара {product_id} не найдена категория в tree.xml"))
